...
import { Code2, Home, User, Briefcase, Menu, X, Link } from 'lucide-react';
...
      initial={{ opacity: 1 }}
      animate={controls}
      className="fixed top-0 w-full z-50 backdrop-blur-sm bg-[rgba(var(--bg-rgb),0.8)] border-b border-blue-500/20"
...
                  className="nav-item group flex items-center space-x-2 text-sm tracking-wider text-gray-300 hover:text-blue-500 transition-colors duration-300"
...
          {/* Mobile Menu Button */}
          <motion.button
            className="lg:hidden text-blue-500 hover:text-blue-400 transition-colors z-[60] p-2"
            whileTap={{ scale: 0.9 }}
...
                key={item.name}
                onClick={() => scrollToSection(item.href)}
                className="flex items-center justify-center space-x-3 text-[rgb(var(--text-rgb))] hover:text-blue-400 transition-colors w-full text-base font-medium py-2 px-4 min-h-[44px] relative group"
...
                  {item.name}.sh
                  <motion.span
                    className="absolute -bottom-1 left-0 w-full h-0.5 bg-blue-500 origin-left transform scale-x-0 transition-transform duration-300"
                    initial={{ scaleX: 0 }}
                    whileHover={{ scaleX: 1 }}
                  />
                </span>
